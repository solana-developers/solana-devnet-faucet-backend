...
import { constants, daysSince, validGithubAccount, validTransactionHistory } from '../faucetEligibility.js';
...
describe('validGithubAccount', () => {
    it('should accept a valid user', async () => {
        const client = makeGithubClient(validUser());
        const result = await validGithubAccount(client, 123);
    ...
    it('should reject with "not found" when GitHub returns 404', async () => {
        ...
        const result = await validGithubAccount(client, 999);
        assert.strictEqual(result.valid, false);
...
describe('validTransactionHistory', () => {
    it('should accept when all counts are under limits', async () => {
        ...
        const result = await validTransactionHistory(db, "1.2.3.4", "wallet1", "gh1");
        assert.deepStrictEqual(result, { valid: true });
    });
